...
<h2>Studenti</h2>
    ...
    created() {
        //elenco_prenotazioni
        const url = 'http://localhost:8080/controller?action=elenco_studenti';
        axios.get(url)
            .then((data) => {
    ...
    methods: {
        elimina(item) {
            console.log(item);
            const url = 'http://localhost:8080/controller?action=remove_studente'
        ...
        addStudent() {
            ...
            const url = 'http://localhost:8080/controller?action=insert_student'
